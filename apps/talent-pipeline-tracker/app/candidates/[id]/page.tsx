...
import { mapStageLabel, mapStatusLabel, STAGE_OPTIONS, STATUS_OPTIONS } from "@/lib/mappers";
...
export default function CandidateDetailPage() {
  ...
  const { record, loading, saving, error, saveFullRecord, saveStatusAndStage, removeRecord } =
  ...
  const [statusStageMessage, setStatusStageMessage] = useState<string | null>(null);

  if (loading) {
    return <main className="p-6 text-slate-700">Loading candidate detail...</main>;
  }
  ...
  if (error || !record) {
    return (
      <main className="p-6">
        ...
        <Link href="/" className="mt-3 inline-block rounded bg-slate-900 px-3 py-2 text-sm text-white">
          Back to list
        </Link>
      </main>
    );
    ...
    <main className="min-h-screen bg-slate-100 px-4 py-8 md:px-8">
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-6">
        <div className="flex items-center justify-between">
          <Link href="/" className="rounded border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700">
            Back to candidates
          </Link>
          <button
            disabled={saving}
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-4">
          <h1 className="text-2xl font-bold text-slate-900">{record.full_name}</h1>
          ...
          <div className="mt-4 grid gap-3 text-sm text-slate-700 md:grid-cols-2">
            ...
            <p>
              <span className="font-semibold">LinkedIn:</span> {record.linkedin_url ?? "N/A"}
            </p>
            <p>
              <span className="font-semibold">CV URL:</span> {record.cv_url}
            </p>
            <p>
              <span className="font-semibold">Years of experience:</span> {record.experience_years}
            </p>
            <p>
            ...
            <p>
              <span className="font-semibold">Status:</span> {mapStatusLabel(record.status)}
            </p>
            <p>
              <span className="font-semibold">Stage:</span> {mapStageLabel(record.stage)}
            </p>
          </div>
          ...
          <div className="mt-4 grid gap-3 rounded-md border border-slate-200 p-3 md:grid-cols-[1fr_1fr_auto]">
            ...
            <button
              ...
              onClick={async () => {
                await saveStatusAndStage(status as RecordStatus, stage as RecordStage);
                setStatus("");
                setStage("");
          ...
          {statusStageMessage && <p className="mt-2 text-sm text-emerald-700">{statusStageMessage}</p>}
